    ...
    <div>
      ...
      {snapshots.length === 0 ? (
        <p className="text-xs text-slate-600 text-center py-4">
          {isPaused ? '快照已在暂停时自动保存' : '暂无快照。当模型额度耗尽时将自动保存'}
        </p>
      ) : (
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          ...
          {snapshot.pausedReason && (
            <p className="text-xs text-amber-400/70 mt-1 truncate">{snapshot.pausedReason}</p>
          )}
        </div>
        <div className="flex gap-1.5 shrink-0">
          <button onClick={onRestore} className="glass-btn-sm">恢复</button>
          <button onClick={onDelete} className="glass-btn-sm text-red-400 hover:bg-red-500/10 hover:border-red-500/30">删除</button>
        </div>
      </div>
    </div>
  );
